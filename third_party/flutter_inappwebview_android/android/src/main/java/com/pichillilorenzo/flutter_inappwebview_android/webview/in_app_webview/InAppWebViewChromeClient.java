...
public class InAppWebViewChromeClient extends WebChromeClient implements PluginRegistry.ActivityResultListener, ActivityResultListener {
  ...
  private Uri getOutputUri(String intentType) {
    ...
    // Prefer MediaStore so the resulting Uri has a display name the page can read.
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
      Uri mediaStoreUri = createMediaStoreEntry(intentType, activity);
  ...
  @Nullable
  private Uri createMediaStoreEntry(String intentType, Activity activity) {
    String prefix = "image";
    String mimeType = "image/jpeg";
    String relativePath = Environment.DIRECTORY_PICTURES + File.separator + "InAppWebView";
    if (intentType.equals(MediaStore.ACTION_VIDEO_CAPTURE)) {
      prefix = "video";
      mimeType = "video/mp4";
      relativePath = Environment.DIRECTORY_MOVIES + File.separator + "InAppWebView";
    }

    String fileName = String.format("%s-%d%s", prefix, System.currentTimeMillis(), intentType.equals(MediaStore.ACTION_IMAGE_CAPTURE) ? ".jpg" : ".mp4");
    ContentValues values = new ContentValues();
    values.put(MediaStore.MediaColumns.DISPLAY_NAME, fileName);
    values.put(MediaStore.MediaColumns.MIME_TYPE, mimeType);
    values.put(MediaStore.MediaColumns.RELATIVE_PATH, relativePath);

    Uri collection = intentType.equals(MediaStore.ACTION_IMAGE_CAPTURE)
            ? MediaStore.Images.Media.EXTERNAL_CONTENT_URI
            : MediaStore.Video.Media.EXTERNAL_CONTENT_URI;
    try {
      return activity.getContentResolver().insert(collection, values);
    } catch (Exception e) {
      Log.e(LOG_TAG, "Unable to create MediaStore entry", e);
      return null;
    }
  }
